...
summary(Neonics$Species.Common.Name, maxsum = 7) #note: sort is not needed with maxsum because maxsum 
                                                 #automatically takes the most frequent levels. n+1 
                                                 #levels need to be specified one level is "other".
...
> Answer: The unique function provides the levels of plots at Niwot Ridge. The summary function provides the count for each level. The summary function therefore provides all of the information the unique function provides and more information (i.e., count). 
...
# at low mass levels.Using ylim, look at the lowest masses to see some of these 
# distributions
ggplot(data = Litter)+
  geom_violin(aes(x = functionalGroup, y = dryMass), 
              drop = FALSE, 
...
> Answer: The violin plots does not well illustrate the distributions. The values are mostly all concentrated in between masses of 0g and 0.02g except for mixed, needles, and twig/branches functional groups having larger distributions. These smaller distributions are not well illustrated due to these scale differences. There are also a couple groups with designated 'outliers'. This is shown on the violin plot as a straight line connecting these outliers to the majority of the data It is not clear how many outliers lie between the largest mass outlier and most of the values. The box plot more clearly represents the distribution of the functional groups at smaller scales

What type(s) of litter tend to have the highest biomass at these sites?

> Answer: Needles tend to have the highest biomass at these sites followed by mixed and twig/branches.
