...
filenames <- dir(here('Data','Raw'),pattern = ("*.csv")) # a character vector of the names of files or directories in the named directory 
print(filenames)
```
...
fix.early.dates <- function(d, cutoff) { #cutoff is a date object
  
       m <- year(d) %% 100  
       # The %% operator does modular division i.e. integer-divides 
       # year(d) by 100 and returns the remainder - strips off the century
...
A factor is a categorical variable. Sometimes numbers are names - and they should be converted to factors. Similar to putting factorsAsStrings = TRUE, they are often names. 
