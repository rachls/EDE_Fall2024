...
The expression function can be used when you want to add mathematical symbols and subscripts mixed with regular letter to labels. Paste0 means no separator between things within parenthesis. Paste you can specify the separator. 
...
---
## A Note on Factors
When we are creating plots, we want characters to be represent as factors/levels, othwerwise ggplot doesn't know waht to do. You can give your levels labels as well. 
...
- Change colour outside the aesthetics is to change the colour of a single layer. Within aesthetics is to categorize. 
...
surface_temp_byday <- PeterPaul.chem.nutrients %>%
filter(depth == 0) %>%
ggplot()+
...
temp_bydate_depth <- PeterPaul.chem.nutrients %>%
