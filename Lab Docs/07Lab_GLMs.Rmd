...
ggplot(EPAair, aes(x = Ozone))+
  geom_density(aes(fill = as.factor(Year), alpha = 0.01))+
...
lm_pm25_o3 <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(lm_pm25_o3)
...
ggplot(EPAair, aes(x = Ozone, y = PM2.5))+
  ...
  geom_text(x=35,
            y=90,
            label = expression(paste("PM2.5 = 0.38384(Ozone) + 15.63824, ", R^{2}, " = 0.1314")))
...
PM2.5.reg <- lm(data = EPAair, PM2.5 ~ Ozone + Month + mean_Lng + mean_Lat)
