...
> Answer: It reiterates the package you are loading (e.g., `>library(here)`) and then states whether there are any conflicts.  
...
> ANSWER: The file can be found in the Metadata folder contained within the Data folder of the EDE_Fall2024 project. The file contains information about naming conventions and data collection, which will facilitate reproducing the data collection or the analysis. The metadata folder is easy to find and kept separate from the raw and processed data to prevent confusion and accidental file changes.
...
```{r Exploring data}
# View(USGS.flow.data) - can't use with knit
# Alternate option: click on data frame in Environment tab

class(USGS.flow.data) #what type of variable is it
colnames(USGS.flow.data) #gives column names
...
str(USGS.flow.data) #structure: gives information on dataframe and columns - can also use glimpse
dim(USGS.flow.data) #dimension: gives row vs. column
...
class(USGS.flow.data$datetime) #is factor... want it to be date time
...
> Answer: They were given the value NA in numeric columns and a blank string in character columns
...
```{r Finding NAs}
summary(USGS.flow.data$discharge.mean) #note the NA values
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #removes any row with missing data in any column
dim(USGS.flow.data)
dim(USGS.flow.data.complete)

mean(USGS.flow.data.complete$discharge.mean)
# mean(USGS.flow.data$discharge.mean) - cannot calculate due to NA values
...
#format is the format the raw dataframe is in.
...
```{r Fixing dates}
# formatting the dates to be character
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

# create a function that will grab the date and fix the first two letters of the year if the year was prior to 1969
...
#format as datetime with the specified format
