...
1.  Discuss the purpose and application of time series analysis (TSA) for environmental data
...
Time series are a special class of dataset, where a response variable is tracked over time. The frequency of measurement and the timespan of the dataset can vary widely. At its most simple, a time series model includes an explanatory time component and a response variable.
...
Mathematically, time series are represented by values Y1, Y2, etc. at times t1, t2, etc. Therefore, Y = F(t).

TSA analyze past behaviour of a variable to predict future behaviour. Causes of time series data variation include: seasons, holidays, and calamities (natural and human-made). It is important for business forcasting and planning future operations

### Components

1.  Time frame: how long can we predict
    -   short, medium, and long term
2.  Trend: general tendency to grow or decline over a long period
3.  Cycle: repetitive up and down movement that repeats over **long** periods of time
    -   ex. business prosperity and decline
4.  Seasonal variation: repetitive up and down movement that repeats over **short** periods of time
    -   ex. climate, weather, customs & traditions
5.  Random variation: erratic, unpredictable movements that don't follow a pattern

#### Trends

Trends can be linear or nonlinear. For a linear trend Yi = Beta0 + Beta1 \* ti + ei. Where, slope is B1, intercept is B0, and ei is error term. We want to find this to detrend the data set. This is easier to observe: Ydetrend = Yt - (B0 + B1t).

##### stochastic vs. deterministic

Trend stationary: Determine the linear trend and then detrend the series.

Stochastics trend: different behaviours over time, has difference-stationarity. Stationarity is achieved by differencing (segmenting) the series.

##### Stationarity Assessment

-   Mann-Kendall test: common for deterministic trend with environmental data, cannot be applied to seasonal data.

    -   Test statistic is S, checks magnitude and significance. More observations needs a higher S

    -   use kendall package

![](images/clipboard-59734963.png)

-   Spearman Correlation: statistical measure of monotonic relationship; relationship does not have to be linear; if one variable increases and so does the other, there is a correlation; similar to pearson

    -   correlation close to 0 = no trend

    -   cor() or cor.test() in stats package (latter also gives significance of coefficient). Default of cor is Pearson

-   Dick Fuller Test: verifies stochasitc (Aka unit root) trend

    ![](images/clipboard-1303973739.png){width="257"}

#### Seasonal Component

Patterns observed within one year. We estimate seasonal trends with Yt = ut + Xt, where E[Xt] = 0. The ut takes 12 different parameters, one for each month, that repeats every 12 observations. The number of seasons may be less than 12.

![](images/clipboard-278913994.png)

### Definitions

-   Stationary data: time series data without a significant upward or downward trend over time

-   Non-stationary data: time series variable with a significant upward or downward trend over time

-   Seasonal data: time series variable exhibiting repeating patterns at regular intervals

-   Correlation: joint vairability of 2 variables; measure of linear dependence between two variables

-   Autocorrelation: measure of dependence between two adjacent values of the same variable

    -   pt,t-h = Corr(Yt,Yt-h) \<- autocorrelation function (ACF)

    -   measures temporal dependency of stochastic (random) process

-   Stationary process: probability laws that govern behaviour do not change over time

    -   White noise series: sequence of independent, identically distributed random variables

-   Partial autocerrelation function (PACF): removes the influence of intermediate variables of an original time and the lag time in an ACF

-   Autoregressive model: a type of time series model that captures the relationship between an observation and several lagged observations (previous time steps). The core idea is that the current value of a time series can be expressed as a linear combination of its past values, with some random noise.

### Plots

Each bar represents a lag. Plotting ACF is another way to identify seasonal data.
...
ggplot(EnoDischarge.late_Components) +
  ...
  geom_line(aes(y = trend, x = Date), color = "cyan") +
...
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = seasonal, x = Date), color = "cyan") +
  geom_hline(yintercept = 0, lty = 2) +
  ylab(expression("Discharge (ft"^3*"/s)"))
